...
class GCNII_RW_full(torch.nn.Module):
    def __init__(self, dataset, args):
        super(GCNII_RW_full, self).__init__()
        data = dataset[0]
        ...
        self.alpha = args.paraA
        self.theta = args.paraB
    ...
    def samp_aggr_layer(self, x, x_0, edge_index, layer_index):
        ...
        if self.training:
            ...
            return (1 - self.alpha) * aggx + self.alpha * x_0
        else:
            aggx = x[batch] * mask[0]
            for i in range(1, self.K+1):
    ...
    def forward(self, data):
        ...
        x = F.relu(self.lins[-2](x))
        x_0 = x.clone()

        for i in range(self.nlayer):
            x = F.dropout(x, p=self.dropout, training=self.training)
            x = self.samp_aggr_layer(x, x_0, edge_index, i)
            self.beta = math.log(self.theta / (i+1) + 1)
            x = (1 - self.beta) * x + self.beta * self.lins[i](x)
            x = F.relu(x)
        x = self.lins[-1](F.dropout(x, p = self.dropout, training = self.training))
        return F.log_softmax(x, dim=1)

class GCNII_RW_mini_G(torch.nn.Module):
    def __init__(self, dataset, args):
        super(GCNII_RW_mini_G, self).__init__()
        data = dataset[0]
        #This modification is for cora only.
        #Please use looped version for other datasets.
        #Maybe better to other datasets, but unnecessary.
    ...
    def samp_aggr_layer(self, x, x_0, edge_index, layer_index, batch):
        mask = self.att[layer_index]
    ...
    def forward(self, data, batch):
        ...
        for i in range(self.nlayer):
            ...
            x = F.dropout(x, p=self.dropout, training=self.training)
            x = self.samp_aggr_layer(x, x_0, edge_index, i, batch)
            self.beta = math.log(self.theta / (i+1) + 1)
            x = (1 - self.beta) * x + self.beta * self.lins[i](x)
            x = F.relu(x)
        x = self.lins[-1](x)
